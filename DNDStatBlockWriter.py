...
def extract_stat_block(text):
    ...
    if actions_match:
        ...
        def make_dice_clickable(action_text):
            ...
            def replace_to_hit(match):
                to_hit_value = match.group(1)  # Extract the "+X" value
                return f'<b onclick="RollItem( {to_hit_value}, \'To Hit\')" style="cursor: pointer;"> +{to_hit_value} </b>'

             # First, replace dice rolls
            action_text = re.sub(r'(\d+d\d+)', replace_dice, action_text)
            # Then, replace "+X to hit"
            action_text = re.sub(r'\+\s*(\d+)\s*to hit', replace_to_hit, action_text, flags=re.IGNORECASE)
